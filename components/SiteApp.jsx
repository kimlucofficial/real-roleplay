...
const RequestAccessHero = memo(function RequestAccessHero({ setPage, onApply }) {
  const cards = [
    ['QUALITY CONTROL', 'Mọi hồ sơ đều được kiểm soát chất lượng trước khi vào thành phố.', Landmark],
...
const INITIAL_WHITELIST_FORM = {
  full_name: '',
  age: '',
  rp_experience: '',
  online_time: '',
  source: 'Kiểm Soát Chất Lượng',
  short_description: '',
  backstory: '',
  why_join: ''
};

const restrictedWhitelistFields = {
  full_name: /[^\p{L}\s]/gu,
  age: /[^0-9]/g,
...
const WhitelistApplicationModal = memo(function WhitelistApplicationModal({ onClose }) {
  const { data: session, status } = useSession();
  const [form, setForm] = useState(INITIAL_WHITELIST_FORM);
  ...
  const onChange = useCallback((e) => {
    ...
    const cleanValue = restrictedWhitelistFields[name]
  ...
  const submit = useCallback(async (e) => {
    ...
    if (!/^[a-zA-Z\s]+$/.test(form.short_description.trim())) {
      setMessage('Tên nhân vật chỉ được dùng chữ không dấu và không dùng số/ký tự đặc biệt.');
      setLoading(false);
      return;
    }
...
function WhitelistPage({ setPage }) {
  ...
  const openApplyPanel = useCallback(() => setShowApplyPanel(true), []);
  const closeApplyPanel = useCallback(() => setShowApplyPanel(false), []);

  return (
    <>
      <RequestAccessHero setPage={setPage} onApply={openApplyPanel} />
      {showApplyPanel && <WhitelistApplicationModal onClose={closeApplyPanel} />}
    </>
  );
}
