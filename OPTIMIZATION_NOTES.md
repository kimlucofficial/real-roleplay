...
## Whitelist resubmit rule
- Người chơi chỉ được nộp lại khi đơn mới nhất đã bị `rejected`.
- Nếu đang `pending`/`review` hoặc đã `approved`, API sẽ chặn nộp đơn mới.
- API dùng MySQL `GET_LOCK()` theo `discord_id` để tránh double-submit/race condition khi bấm gửi nhiều lần hoặc nhiều tab cùng lúc.
